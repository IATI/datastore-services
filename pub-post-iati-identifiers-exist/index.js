...
async function testIatiIdentifiers(queryUrl, identifiers, maxRecords) {
    const iatiIdentifiersQuery = identifiers
        .map((identifier) => `iati_identifier_exact:${JSON.stringify(identifier)}`)
        .join(' ');
    ...
    const solrJsonResponse = await query(
        queryUrl,
        'JSON',
        false,
        false,
...
export default async function pubIatiIdentifiersExist(context, req) {
    try {
        ...
        if (toString.call(body.iati_identifiers) !== '[object Array]') {
            context.res = getJsonErrorResponse(
                400,
        ...
        if (body.iati_identifiers.length > 0) {
            ...
            for (let batchNum = 0; batchNum <= numBatches; batchNum += 1) {
                const currentBatch = uniqueIdentifiers.slice(
                    batchNum * BATCH_SIZE,
                    batchNum * BATCH_SIZE + BATCH_SIZE,
                );
                const identifiersFound = await testIatiIdentifiers(
                    queryUrl,
                    currentBatch,
                    BATCH_SIZE,
                );
                const identifiersNotFound = Object.fromEntries(
                    currentBatch.filter((x) => !(x in identifiersFound)).map((v) => [v, {}]),
                );
            ...
            responseBody.unique_iati_identifiers_found = Object.keys(
                responseBody.iati_identifiers_found,
            ).length;
            responseBody.total_iati_identifier_occurrences = Object.values(
                responseBody.iati_identifiers_found,
            ).reduce((acc, val) => acc + val.count, 0);
            responseBody.num_iati_identifiers_not_found = Object.keys(
                responseBody.iati_identifiers_not_found,
            ).length;
        }
